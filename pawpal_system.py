...
class Scheduler:
    ...
    def _is_duplicate_task_for_pet(self, task: Task, pet: Pet) -> bool:
        """Check whether adding `task` would duplicate an existing task for `pet`.

        Recurring generated tasks are deduplicated by `(parent_task_id, generated_for)`.
        Manual tasks are deduplicated by matching type, normalized description,
        duration, and exact due datetime.
        """
    ...
    def _should_generate_for_day(self, template: Task, target_day: date) -> bool:
        """Return whether `template` should create an instance on `target_day`.
    ...
    def _next_weekday(from_day: date) -> date:
        """Compute the next business day (Mon-Fri) after `from_day`."""
    ...
    def _spawn_instance_from_template(self, template: Task, owner: Owner, target_day: date) -> None:
        """Create one non-template task from `template` for `target_day`.

        The method is idempotent for a given template/day pair and does nothing
        when that occurrence already exists or when the pet cannot be resolved.
        """
    ...
    def _spawn_recurring_tasks_for_date(self, owner: Owner, target_day: date) -> None:
        """Generate all due recurring instances for `owner` on `target_day`.

        Only tasks marked as templates are considered, and each template is
        evaluated by `_should_generate_for_day` before spawning.
        """
    ...
    def complete_task(self, task_id: int, owner: Owner) -> None:
        """Mark a task complete and pre-generate the next recurrence when needed.

        For daily templates, this creates tomorrow's instance.
        For weekly templates, this creates the next weekday instance.
        Non-recurring tasks are only marked complete.
        """
    ...
    def undo_complete_task(self, task_id: int) -> None:
        """Undo completion and remove the previously pre-spawned next instance.

        This reverts `is_completed` and deletes the corresponding future
        recurrence generated by `complete_task` if it is still incomplete.
        """
        task = self.get_task(task_id)
    ...
    def filter_tasks(
        ...
        """Return owner's tasks due on `due_date` with optional extra filters.

        Optional filters:
        - `is_completed`: include only tasks matching completion status.
        - `pet_name`: case-insensitive match on the task's pet name.
        """
    ...
    def get_today_tasks(self, owner: Owner) -> List[Task]:
        """Return unique actionable tasks due today or earlier for `owner`.

        Before filtering, this materializes today's recurring tasks. Returned
        tasks exclude templates and completed items.
        """
    ...
    def generate_schedule(self, owner: Owner, available_mins: int) -> List[Task]:
        """Build a duration-constrained schedule using deterministic ranking.

        Ranking preference is:
        1. overdue before not overdue
        2. high -> medium -> low priority
        3. earlier due time
        4. shorter duration

        Tasks are greedily selected in ranked order while total duration stays
        within `available_mins`.
        """
